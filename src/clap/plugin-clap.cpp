...
template <bool multiOut> struct SynthTemplate : public plugHelper_t, sst::clap_juce_shim::EditorProvider
{
    SynthTemplate(const clap_host *h) : plugHelper_t(getDescriptor(), h)
    {
  ...
  protected:
    ...
    bool handleEvent(const clap_event_header_t *nextEvent)
    {
        ...
        if (nextEvent->space_id == CLAP_CORE_EVENT_SPACE_ID)
        {
            ...
            default:
            {
                DEBUG_LOG("Unknown inbound event of type " << nextEvent->type);
            }
            break;
            }
        }
  ...
  public:
    ...
    {
        auto res = std::make_unique<david303ttl::synthtemplate::ui::PluginEditor>(
        ...
        res->onZoomChanged = [this](auto f)
        {
            if (_host.canUseGui() && clapJuceShim->isEditorAttached())
            {
                ...
                guiSetSize(david303ttl::synthtemplate::ui::PluginEditor::edWidth * s,
                           david303ttl::synthtemplate::ui::PluginEditor::edHeight * s);
                _host.guiRequestResize(david303ttl::synthtemplate::ui::PluginEditor::edWidth * s,
                                       david303ttl::synthtemplate::ui::PluginEditor::edHeight * s);
            }
        };
...
const clap_plugin *makePlugin(const clap_host *h, bool multiOut)
{
    if (multiOut)
    {
        auto res = new david303ttl::synthtemplate::clapimpl::SynthTemplate<true>(h);
        return res->clapPlugin();
    }
    else
    {
        auto res = new david303ttl::synthtemplate::clapimpl::SynthTemplate<false>(h);
        return res->clapPlugin();
    }
}
} // namespace david303ttl::synthtemplate
