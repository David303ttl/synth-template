...
static constexpr sheet_t::Class PatchMenu("synth.patch-menu");
    ...
    : jcmp::WindowPanel(true), audioToUI(atou), mainToAudio(utoa), clapHost(h)
{
    ...
    defaultsProvider = std::make_unique<defaultsProvder_t>(
        presetManager->userPath, "SynthTemplate", defaultName,
        [](auto e, auto b) { DEBUG_LOG("[ERROR]" << e << " " << b); });
    setSkinFromDefaults();
...
void PluginEditor::idle()
{
    ...
    while (aum.has_value())
    {
        ...
        {
            DEBUG_LOG_ONCE("Implement update voice count");
        }
        ...
        {
            DEBUG_LOG("Ignored patch message " << aum->action);
        }
        aum = audioToUI.pop();
    }
}
...
void PluginEditor::paint(juce::Graphics &g)
{
    ...
    auto bi = std::string("Windows ") + sst::plugininfra::VersionInformation::git_commit_hash;
...
void PluginEditor::showPresetPopup()
{
    ...
    p.addItem("Get the Source",
              []() {
                  juce::URL("https://github.com/David303ttl/synth-template/")
...
void PluginEditor::parentHierarchyChanged()
{
    ...
    if (swr)
    {
        ...
        {
            DEBUG_LOG("Enabling software rendering engine");
...
bool PluginEditor::toggleDebug()
{
    ...
    if (debugLevel <= 0)
        ...
        debugLevel = -1;
    DEBUG_LOG("Started debug session");
    DEBUG_LOG("If you are on windows and you close this window it may end your entire session");
    return debugLevel > 0;
}
