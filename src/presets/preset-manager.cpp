...
PresetManager::PresetManager(const clap_host_t *ch) : clapHost(ch)
{
    ...
    {
        userPath =
            sst::plugininfra::paths::bestDocumentsVendorFolderPathFor("David303ttl", "SynthTemplate");
        if (clapHost)
    ...
    {
        DEBUG_LOG("Unable to create user dir " << e.what());
    }

    try
    {
        auto fs = cmrc::synth_patches::get_filesystem();
